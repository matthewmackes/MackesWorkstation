#!/bin/sh
# install-helpers/lint-design-doc-sync.sh — pre-commit gate
# (design-doc -> worklist sync).
#
...
# for sessions, going unnoticed until an audit catches them. This
# lint replaces the catch-up with at-write-time enforcement.
#
# Per CLAUDE.md section 2 conventions (single-worklist rule:
# design docs are not a parallel worklist — every actionable item
# lifts into docs/PROJECT_WORKLIST.md) + section 3 Definition of
# Done.
#
# NOTE: docs/design/ does not exist yet in this monorepo, so this
# gate is a no-op (it short-circuits clean on every commit) until
# the first design doc lands. That is intentional.
#
...
echo "lint-design-doc-sync.sh: design-doc -> worklist sync violation."
echo
...
echo
echo "Per CLAUDE.md section 2 (design docs are not a parallel"
echo "worklist), design-doc edits must lift actionable items into"
echo "the worklist at write-time. Either:"
