...
from mackes.birthright import (
    ...
    apply_flathub, apply_fleet, apply_fonts, apply_maximize_all,
    apply_panel_layout, apply_plymouth, apply_qnm, apply_remote_desktop,
...
class ApplyPage(Gtk.Box):
    ...
    def run(self) -> None:
        ...
        steps = [
            ...
            ("Conky HUD",         lambda: apply_conky(merged)),
            ("Maximize windows",  lambda: apply_maximize_all(merged)),
            ("Mesh clipboard",    lambda: apply_clipboard_daemon(merged)),
            ("Quick Network Mesh", lambda: apply_qnm(merged)),
            # ---------------------------------------------------------------
