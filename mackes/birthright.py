...
_MESH_SWAY_DIR = "mesh-storage/sway"
_MESH_SWAY_CONF_NAME = "shared.conf"
_MESH_INCLUDE_FRAG_NAME = "50-mesh-shared.conf"

_MESH_INCLUDE_CONTENT = """\
# MDE mesh-storage sway include (SWAY-8/Q52) — generated by mded.
# DO NOT EDIT — managed by mded at first login.
# When mesh-storage is mounted the wildcard picks up shared.conf
# and any peer-pushed fragments. When offline sway silently skips
# the empty glob.
include ~/.local/share/mde/mesh-storage/sway/*.conf
"""

_MESH_STARTER_CONTENT = """\
# MDE shared sway config — GFS-replicated across the mesh (SWAY-8/Q52).
# This file is propagated to every peer via LizardFS mesh-storage.
# Edit via Workbench > System > Window Manager > Shared config.
# Changes here take effect on all peers within the next mded poll cycle.
"""


def apply_sway_mesh_config_link(_preset: Preset) -> List[str]:
    """Set up the GFS-replicated sway config infrastructure (SWAY-8/Q52).

    Creates:
    - ~/.local/share/mde/mesh-storage/sway/shared.conf  (starter, idempotent)
    - ~/.config/sway/config.d/50-mesh-shared.conf       (include fragment)

    The include fragment uses a glob (`mesh-storage/sway/*.conf`) so
    sway silently skips it when mesh-storage is not mounted — no errors
    on offline peers.
    """
    import pwd
    ...
    if not user or user == "root":
        actions.append("sway-mesh: no primary user in environment; skipped")
        log_action(actions[-1])
        return actions
    ...
    try:
        ...
        actions.append(f"sway-mesh: user '{user}' not in /etc/passwd; skipped")
        log_action(actions[-1])
        return actions

    home = Path(pw.pw_dir)

    # 1. Starter shared.conf in mesh-storage (idempotent — preserve edits).
    mesh_dir = home / ".local" / "share" / "mde" / _MESH_SWAY_DIR
    shared_conf = mesh_dir / _MESH_SWAY_CONF_NAME
    try:
        mesh_dir.mkdir(parents=True, exist_ok=True)
        os.chown(str(mesh_dir), pw.pw_uid, pw.pw_gid)
        if not shared_conf.exists():
            shared_conf.write_text(_MESH_STARTER_CONTENT, encoding="utf-8")
            os.chown(str(shared_conf), pw.pw_uid, pw.pw_gid)
            actions.append(f"sway-mesh: created {shared_conf}")
            log_action(actions[-1])
        else:
            actions.append(f"sway-mesh: {shared_conf} already present; preserved")
            log_action(actions[-1])
    except OSError as e:
        actions.append(f"sway-mesh: mesh-storage dir setup failed: {e}")
        log_action(actions[-1])
        return actions

    # 2. Include fragment in config.d (always write — safe to overwrite
    #    because the content is fixed and generated by mded).
    config_d = home / ".config" / "sway" / "config.d"
    frag = config_d / _MESH_INCLUDE_FRAG_NAME
    try:
        config_d.mkdir(parents=True, exist_ok=True)
        os.chown(str(config_d), pw.pw_uid, pw.pw_gid)
        if not frag.exists() or frag.read_text(encoding="utf-8") != _MESH_INCLUDE_CONTENT:
            frag.write_text(_MESH_INCLUDE_CONTENT, encoding="utf-8")
            os.chown(str(frag), pw.pw_uid, pw.pw_gid)
            actions.append(f"sway-mesh: wrote {frag}")
            log_action(actions[-1])
        else:
            actions.append(f"sway-mesh: {frag} up-to-date; skipped")
            log_action(actions[-1])
    except OSError as e:
        actions.append(f"sway-mesh: include fragment write failed: {e}")
        log_action(actions[-1])
        return actions
...
# in the INST epic. The Hyprland baseline step was removed entirely
# (SWAY-9.d) — Hyprland was retired 2026-05-28 (sway-native reversal).
...
STEP_FUNCS: "dict[str, Callable[[Preset], List[str]]]" = {
    ...
    "sway-mesh-config":  apply_sway_mesh_config_link,
...
_DESKTOP_EXTRA: tuple[str, ...] = (
    ...
    "apps", "panel-layout", "sway-config", "sway-mesh-config", "display-manager", "boot-splash",
