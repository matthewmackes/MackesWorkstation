...
fn to_mode(m: &RawMode) -> Mode {
    Mode {
        width: m.width,
        height: m.height,
        refresh_mhz: (m.refresh * 1000.0).round() as i32,
    }
}
...
impl DesiredOutput {
    fn from(o: &Output) -> Self {
        let cur = o.current.unwrap_or(Mode {
            width: o.rect_w,
            height: o.rect_h,
            refresh_mhz: 60_000,
        });
