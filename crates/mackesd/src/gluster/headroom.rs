//! GF-12.2 (v5.0.0) — gluster-brick headroom pre-flight check.
//!
//! Pure-function `check(brick_dir, xdg_dirs)` walks the listed
//! XDG directories, sums their on-disk byte sizes, queries the
//! filesystem free space at `brick_dir`, and reports whether
//! the brick has enough headroom for the mesh-home volume to
//! safely accept replication overhead (target: ≥ 1.5× the
//! sum of XDG content).
//!
//! Operator-invokable via `mackesd preflight gluster-headroom`
//! (added in this commit). The Workbench "Mesh Storage" panel
//! (GF-8.x) will surface the same report as a banner once it
//! lands; the CLI lets operators run the check from a
...
/// Default brick directory the v5.0.0 GlusterFS substrate
/// uses (per the design lock § 3.2 — `/var/lib/gluster/
/// bricks/<volume>/`). `check()` takes the brick dir as an
/// arg so tests + dev rigs use a tempdir; production callers
/// pass this constant.
pub const DEFAULT_BRICK_DIR: &str = "/var/lib/gluster/bricks";

/// Headroom multiplier — the worklist locks 1.5× of the sum
/// of XDG content as the WARN threshold. Below this, the
/// pre-flight reports WARN; at or above, OK.
pub const HEADROOM_MULTIPLIER: f64 = 1.5;

/// The five XDG dirs that the mesh-home FUSE mount owns
/// once GF-3.3 lands. The pre-flight sums the bytes
/// currently sitting in these locations on the local peer.
...
pub enum Verdict {
    /// Brick has enough headroom — install / upgrade is safe.
    Ok,
    /// Brick headroom is below `1.5 × xdg_bytes`. The
    /// operator can still install; mesh-home will accept
    /// writes until the quota (Q16 — `0.8 × min(free brick)`)
    /// pinches.
    Warn,
    /// Brick directory doesn't exist at all (glusterfs-server
    /// not installed yet, or the brick mountpoint is missing).
    /// Treated as Warn semantically — operator should fix
    /// before mesh-home bootstrap.
    NoBrick,
}

/// Headroom report — what the CLI prints + what the panel
/// banner reads from.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct HeadroomReport {
    /// OK / Warn / NoBrick.
    pub verdict: Verdict,
    /// Brick directory examined.
    pub brick_dir: PathBuf,
    /// Free bytes at the brick filesystem. `0` when
    /// `verdict == NoBrick`.
...
impl HeadroomReport {
    ...
    pub fn summary(&self) -> String {
        match self.verdict {
            Verdict::NoBrick => format!(
                "[NoBrick] {brick} missing — install glusterfs-server first",
                brick = self.brick_dir.display()
            ),
            Verdict::Ok => format!(
                "[OK] brick free {free_mb} MB ≥ required {req_mb} MB (XDG {xdg_mb} MB × 1.5)",
                free_mb = self.brick_free_bytes / 1_048_576,
                req_mb = self.required_bytes / 1_048_576,
                xdg_mb = self.xdg_total_bytes / 1_048_576,
            ),
            Verdict::Warn => format!(
                "[WARN] brick free {free_mb} MB < required {req_mb} MB (XDG {xdg_mb} MB × 1.5) — \
                 mesh-home will accept writes but quota will pinch",
                free_mb = self.brick_free_bytes / 1_048_576,
                req_mb = self.required_bytes / 1_048_576,
                xdg_mb = self.xdg_total_bytes / 1_048_576,
            ),
...
///
/// `brick_dir` is the candidate brick parent (default
/// `/var/lib/gluster/bricks`). `xdg_dirs` is the list of
/// directories whose content will move into mesh-home —
/// pass [`default_xdg_dirs($HOME)`].
///
/// Pure-ish: walks the filesystem (so not strictly pure) but
/// returns a fully-self-described [`HeadroomReport`] with
/// no side effects beyond stat calls.
#[must_use]
pub fn check(brick_dir: &Path, xdg_dirs: &[PathBuf]) -> HeadroomReport {
    if !brick_dir.exists() {
        return HeadroomReport {
            verdict: Verdict::NoBrick,
            brick_dir: brick_dir.to_path_buf(),
            brick_free_bytes: 0,
            xdg_sizes: xdg_dirs
                .iter()
    ...
    let free = filesystem_free_bytes(brick_dir).unwrap_or(0);
...
/// Recursive byte-size walk. Symlinks are NOT followed (we
/// want the local on-disk footprint). Per-entry errors are
/// silently skipped — pre-flight is best-effort.
fn dir_size_bytes(dir: &Path) -> u64 {
    ...
    for entry in entries.flatten() {
        ...
        // Symlinks counted as 0 (their target is somewhere
        // else on disk and may double-count if we followed).
    }
    total
}
...
/// parses the second line. Returns `None` on shellout
/// failure or unparseable output — the workspace forbids
/// `unsafe_code`, so direct `statvfs` via libc isn't on the
/// table.
fn filesystem_free_bytes(path: &Path) -> Option<u64> {
    ...
    // Line 1 is the "Avail" header; line 2 is the byte count.
...
mod tests {
    ...
    #[test]
    fn no_brick_dir_returns_no_brick_verdict() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let missing = tmp.path().join("does-not-exist");
        let report = check(&missing, &[]);
        assert_eq!(report.verdict, Verdict::NoBrick);
        assert_eq!(report.brick_free_bytes, 0);
    ...
    fn empty_xdg_dirs_yields_zero_required_bytes() {
        ...
        assert_eq!(report.required_bytes, 0);
        // Free > 0 on the tempdir's filesystem; required = 0,
        // so verdict is Ok.
    ...
    fn summary_lines_format_per_verdict_shape() {
        let no_brick = HeadroomReport {
            verdict: Verdict::NoBrick,
            brick_dir: PathBuf::from("/missing"),
            brick_free_bytes: 0,
            xdg_sizes: vec![],
            xdg_total_bytes: 0,
            required_bytes: 0,
        };
        assert!(no_brick.summary().starts_with("[NoBrick]"));

        let ok = HeadroomReport {
            verdict: Verdict::Ok,
            brick_dir: PathBuf::from("/var/lib/gluster/bricks"),
            brick_free_bytes: 10 * 1_073_741_824,
            xdg_sizes: vec![],
        ...
        let warn = HeadroomReport {
            ...
            brick_dir: PathBuf::from("/var/lib/gluster/bricks"),
            brick_free_bytes: 1_073_741_824,
            xdg_sizes: vec![],
