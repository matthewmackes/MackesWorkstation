...
%prep
# E8.5 (2026-06-05) — the source tarball is now produced by `git archive`
# with the %{name}-%{version} prefix (mde-core-<version>/), so a plain
# %autosetup uses the default name. (The historical python-sdist
# mackes_shell-<version>/ prefix + `-n` tripped a double-setup on rpm 4.20.)
%autosetup
...
# Runs after %autosetup so it checks the unpacked source root.
if [ ! -s DISCLAIMER.md ]; then
    echo "ERROR: DISCLAIMER.md is missing or empty — refusing to build the RPM." >&2
    echo "       The educational-mission disclaimer is a release pre-flight gate" >&2
    echo "       (CLAUDE.md §7 / E8.5). Restore a non-empty DISCLAIMER.md." >&2
    exit 1
fi
...
# ANIM-7.a — idle dim/restore helper (Q63).
install -D -m 0755 bin/mde-idle-dim         %{buildroot}%{_bindir}/mde-idle-dim
