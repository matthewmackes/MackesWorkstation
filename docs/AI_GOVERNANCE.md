...
**Imported into the MackesWorkstation monorepo:** 2026-06-03 (E0)
**Supersedes:** `docs/PLATFORM_DESIGN_BRIEF.md` (kept as legacy reference)
**Authority:** Memory > **root `CLAUDE.md`** > **this doc** >
`docs/MACKES-WORKSTATION-PLAN.md` + `docs/design/*.md` >
`docs/PROJECT_WORKLIST.md` body (newer locks always win per §0.67 / CLAUDE.md §0)

This is the **platform identity + architectural compass** for AI design partners.
...
> MDE-Retro + the KDE Connect host). Its **mesh / Bus / storage / security / naming /
> trust / AI-collaboration locks all STAND** — that platform substrate is exactly what
> the monorepo runs underneath. But several locks were **overridden by the fusion plan**
> (`docs/MACKES-WORKSTATION-PLAN.md` §0). Where this doc and the plan disagree, **the
> plan + root `CLAUDE.md` win** (newer + higher authority). The monorepo overrides:
>
...
> | Skills = `plan` / `ship` / `release` (Q87) | live `.claude/skills/` = **`plan` · `ship` · `release` · `audit` · `preview`** | §10 |
> | Co-attribution model id `Opus 4.7` | **`Opus 4.8 (1M context)`** | §10 |
> | RPM split `mde-core` / `mde-desktop` / `mde-xorg` | one spec, install-time role chooser → `mde-core` / `mde-headless` / `mde-desktop` (Lighthouse ⊂ Server ⊂ Workstation); disclaimer pre-flight gate | §9.1 |
> | Python pre-commit gates (ruff/CSS/voice) + `mackes/` paths | a **Rust** tree under `crates/**/src` (no `rust/`, no `mackes/` Python); gates = `cargo build/test/clippy/fmt` | §8 |
>
> The platform **product/code naming** (MackesDE for Workgroups / MackesDE / MDE; the
> `mde-*` + `mackes-*` crate prefixes; `mded`/`mackesd`; Birthright) is **unchanged**.
> The §0 master rule **"Secure, Simple, No-Fixed-Center Workgroup"** is unchanged and
> still governs. The reconciliation rows are mirrored in §14 (Supersessions).
...
> **[MONOREPO] Storage = LizardFS, not Gluster.** "Gluster mesh-home" below is the
> original lock; it was swapped **wholesale** to **LizardFS `mesh-storage`** (2026-05-29,
> MESHFS-*; `docs/design/v5.0.0-mesh-storage-lizardfs.md`) — Gluster never shipped, no
> migration. Read "Gluster mesh-home"/"`mesh-home`" as "LizardFS `mesh-storage`" and the
> arbiter brick as a LizardFS chunk/shadow throughout this doc. (See the §0 banner + §14.)

| Substrate | Holds | Mount |
|---|---|---|
| LizardFS `mesh-storage` (one volume; chunk bricks + HA shadows on peers) | XDG files + MDE-Workgroup coordination | `~/Documents` + `~/Pictures` + `~/Music` + `~/Videos` + `~/Downloads` + `~/.mde-mesh/<peer>/` |
| Bus (ntfy brokers + mesh-storage persistence + per-topic file tree) | Events + clipboard + audit + notifications | `~/.local/share/mde/bus/<topic>/<ulid>.json` |
...
> `palette.rs`**. The Workbench tree retires Python `mackes/workbench/` → Iced
> `crates/workbench/mde-workbench` (this part STANDS). Read the rows below as MDE history.
...
> **[MONOREPO] Version is `v10.0.0`.** The fusion plan locks one workspace version
> across the whole monorepo: `[workspace.package] version = "10.0.0"` (plan §0 Q7;
> "MackesDE for Workgroups" = MDE releases as **v10.0.0**). One git tag
> `MackesWorkstation-v10.0.0`. Read the `v5.0.0` wording below — and every "the v5.0.0
> cut" reference elsewhere in this doc — as **"the v10.0.0 cut"**. The SemVer-continuity
> rationale below still holds (don't present a lower number to dnf/peers); v10 simply
> continues it past the v5 line.
...
| Co-attribution | Always include trailer + exact model identifier (**Opus 4.8 (1M context)** / Sonnet 4.6 / Haiku 4.5) | Q85 (model id updated 2026-06-03) |
| Session attribution | `[>] session=<id>` on every claimed task | Q86 |
| Skills | **[MONOREPO] live set = 5**: `plan` (design/survey/worklist), `ship` (worklist drain), `release` (cut/tag), `audit` (integrity sweep), `preview` (visual accuracy harness). Q87's "consolidate to 3" is extended by MDE-Retro's Rust `audit` + `preview`. | Q87 + MDE-Retro |
...
> **[MONOREPO] Roadmap is the E0–E8 epic sequence.** In `MackesWorkstation` the
> release plan is the **eight ordered epics E0–E8** in `docs/MACKES-WORKSTATION-PLAN.md`
> §11, not the standalone-MDE `v5.0.0` cut gate below. **E0 (monorepo merge) is
> complete** (this repo builds green). The v5.0.0 scope items below (C1–C21 / the master
> inventory) **fold into v10.0.0** and are distributed across E1–E8 (e.g. C4 mesh-storage
> → E3; C5 Bus → E0/substrate; C8/C9 print+firewall, C10 compute → Workbench roles in
> E6; the shell/portal work → E4/E5). The **"ship everything / no incomplete releases"
> spirit STANDS** — the RPM (E8) is **held until every feature is §3-complete**, and HW
> bench is post-release. Read C1–C21 as the *content* of v10.0.0, sequenced by E0–E8.
...
1. **Root `CLAUDE.md`** (operational rules — highest after memory)
2. **This doc** (governance — platform identity + compass)
3. `~/.claude/projects/-home-mm-MackesWorkstation/memory/MEMORY.md` (operator preferences)
4. `docs/MACKES-WORKSTATION-PLAN.md` (the fusion decisions + E0–E8 roadmap) and, once
   execution past E0 begins, `docs/PROJECT_WORKLIST.md` — start with the `[ ] Open`
   section matching your task; look for `[>] session=<id>` markers to avoid collisions
5. Specific `docs/design/<epic>.md` only if doing epic-specific work
   — including a `security-posture.md` when touching public ports, the Bus
   surface, capabilities, or `{{exec}}` Tera templates (§7 expansion)

Live skills: `plan` · `ship` · `release` · `audit` · `preview` (`.claude/skills/`).
...
**[MONOREPO] Supersessions from the fusion (2026-06-03, E0):**
...
| §9.1 RPM split `mde-core`/`mde-desktop`/`mde-xorg` | one spec + install-time role chooser → `mde-core`/`mde-headless`/`mde-desktop` (Lighthouse ⊂ Server ⊂ Workstation) | Plan §0 Q49 + §12 |
| §10 Q87 skills "consolidate to 3" | live set = 5 (`plan`/`ship`/`release` + Rust `audit`/`preview`) | MDE-Retro skill set carried forward |
| §10 Q85 model id `Opus 4.7` | `Opus 4.8 (1M context)` | Current model |
| Authority "`.claude/CLAUDE.md`" | the rulebook is at the **repo root** (`CLAUDE.md`), not `.claude/CLAUDE.md` | Monorepo layout |

The §3.1/§4 Gluster→LizardFS swap was already locked pre-import (MESHFS-*, 2026-05-29);
it is restated here only because parts of the body still say "Gluster".
