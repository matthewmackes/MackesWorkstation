...
- [ ] **OV-test-flake-1: v2.6 — `dbus::tests::focus_handler_*` shared-global flake.** **As** an mde-workbench maintainer, **I want** `cargo test -p mde-workbench --lib` to pass in parallel (the default), **so that** the test signal isn't masked by test-isolation noise. **Acceptance:** removing the `--test-threads=1` workaround keeps the suite at 609 passing. **Implementation notes:** the failing pair (`focus_handler_trims_surrounding_whitespace`, `focus_handler_writes_into_pending_slot`) both write to a process-global `PendingFocus` slot at `crates/mde-workbench/src/dbus.rs`; other tests that read/write the same slot in parallel corrupt the expected value. Either (a) hold a `Mutex<()>` across both tests to serialize, or (b) make `PendingFocus` per-test via thread-local + a test-only accessor. Icon: none (test-only).
- [ ] **OV-test-flake-2: v2.6 — `nebula_ca_backup::tests::backup_path_for_mirrors_bundle_convention` asserts the wrong filename.** **As** a mackesd maintainer, **I want** `cargo test -p mackesd --features async-services --lib` to pass without an unrelated failure, **so that** CI signal stays clean. **Acceptance:** the test passes with the current `nebula_ca_backup` impl. **Implementation notes:** the test asserts the backup file is named `ca-backup.enc` (per the original NF-18.4 design), but the implementation at `crates/mackesd/src/workers/nebula_ca_backup.rs` writes `state-backup.enc`. Decide which name is correct (the design doc, the spec, or the impl) and reconcile. Likely the test was written before the rename and never updated. Icon: none (test-only).
