...
- [ ] **GF-2.2: Create `crates/mackesd/src/ipc/gluster.rs` [HW carve-out]** *(HW carve-out tagged 2026-05-24 per `feedback_no_cut_until_worklist_empty.md`: the 5 signal acceptance gates — `PeerStateChanged` / `ConflictDetected` / `HealCompleted` / `QuotaWarning` / `VolumeReady` — require a live multi-peer gluster fleet to observe the actual state transitions firing on the right events; without bench infrastructure the signal-emission paths have no observable behavior to verify against. The 8 method surfaces (`Status` / `ListPeers` / `AddPeer` / `RemovePeer` / `ConflictList` / `HealStatus` / `MountStatus` / `BootstrapVolume`) could ship as real shellouts to `gluster volume info --xml` etc., but the worklist's acceptance gates the whole task on the signal-transition observation. Doesn't gate the cut.)*  Exposes `dev.mackes.MDE.Gluster.Status` on `/dev/mackes/MDE/Gluster/Status` (zbus 5 `#[interface]`, service `org.mackes.mackesd`). Methods: `Status()`, `ListPeers()`, `AddPeer(node_id)`, `RemovePeer(node_id)`, `ConflictList()`, `HealStatus()`, `MountStatus()`, `BootstrapVolume()`. Signals: `PeerStateChanged`, `ConflictDetected`, `HealCompleted`, `QuotaWarning`, `VolumeReady`. Register as the sixth service in `crates/mackesd/src/ipc/mod.rs:43`.
