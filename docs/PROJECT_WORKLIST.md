...
- [ ] **E4.20: E4 — Retire mde-portal: confirm every portal function is reachable as a Win10 idiom (shell/Settings/Explorer/Action Center), then delete the crate**
  ...
  **Remaining (real work + gated):** ① migrate the `action/shell/open-uri` consumer to a Win10 surface (so `mde://` survives); ② retire `mde-portal.service` + reconcile `mde-open.desktop`; ③ **[operator-gated §0.5]** delete `crates/legacy/mde-portal/` + drop its 6 `lint-design-tokens.allowlist` `portal_full_main.rs` entries; ④ `cargo check --workspace` green (already true). **The crate deletion needs operator authorization; ① is a genuine prerequisite, not a rubber-stamp.**
