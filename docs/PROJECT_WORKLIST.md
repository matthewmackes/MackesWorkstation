...
- [ ] **CR-3.b: v2.6 — Extract `panel_chrome::object_card` to a shared crate so CR-4..CR-8 consumers outside mde-workbench can use it.** *(filed 2026-05-25 as a CR-4 unblock)*
  ...
  **Acceptance:**
  - [ ] Create a new shared crate `crates/mde-iced-components/` (or add an `iced-widgets` feature flag to `mde_theme` that pulls in full `iced` and the widget builders) that hosts `pub fn object_card(card: ObjectCard, palette: Palette) -> Element<'_, Message>` + the inline overlay/alpha helpers.
  - [ ] `panel_chrome::object_card` re-exports from the new location so existing mde-workbench call sites stay unchanged (or move the entire body and re-export the symbol).
  - [ ] mde-files + mde-popover + any other CR-4..CR-8 consumer crate gains the new dep and imports `mde_iced_components::object_card` (or `mde_theme::object_card` with the `iced-widgets` feature) — no per-crate duplication.
  - [ ] All 14 panel_chrome tests + 116 mde-theme tests still pass after the move.
  **Implementation notes:**
    - The cleaner architecture is a new crate (`mde-iced-components`) since mde_theme's design lock specifically excludes the toolkit dep ("the toolkit dep doesn't leak into this crate" — see `crates/mde-theme/src/components/mod.rs`). Adding a feature flag inside mde_theme would muddy that lock.
    - First identified during CR-4 implementation 2026-05-25: mde-files's `folder_row_button` could not consume `panel_chrome::object_card` because mde-files doesn't depend on mde-workbench (and shouldn't — they're peer crates).
    - Blockers: CR-3 ✓ (shipped 2026-05-25 in commit `4f17a7a6`).
- [ ] **CR-4: v2.6 — mde-files visual retrofit (layout unchanged, look swapped). [Blocked on CR-3.b]**
