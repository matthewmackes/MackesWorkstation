...
- [>] **E4.1: E4 — Win10 era foundation: Theme::Windows10 + win10(rgb) remap (accent #0078d4/#2899f5) through palette::color(), font::family(), state.rs, main.rs startup; bottom panel anchor; Display > Appearance "Windows 10" picker; pin in checklist.rs** [M1] *(session=ship-2026-06-04 — CONFLICT FOUND, see note)*
  **⚠ STALE vs the code (2026-06-04) — mostly delivered by the "MackesDE rebrand", which superseded this task's accent design.** Tracing the shell: `Theme::Windows10` is already a fully runtime-reachable, state-persisted era. **#2 (bottom anchor) DONE** (`panel.rs:312` `else if is_windows10()` → 40 px bottom bar). **#3 (Appearance picker + restart) DONE** (`display.rs` theme picker maps `Windows10`↔state `"windows10"` + rewrites labwc themerc; `main.rs:148` reads it at startup; `state.rs:735` round-trips) — **but the picker is labeled "MackesDE 10", not "Windows 10".** **#1 (the `#0078d4`/`#2899f5` accent) was DELIBERATELY RETIRED** — `palette.rs:425-427` + `hex()` + `greeter.rs:6` document: *"Windows 10 shares Carbon's coloring + accent verbatim … its former blue accent + accent picker were retired in the MackesDE rebrand"* (`palette::win10` deleted). So `color()` aliases `Theme::Windows10 => carbon(rgb)`. **#4 (checklist pin) genuinely missing.** Per CLAUDE.md authority (newer code-lock > worklist body) the rebrand wins, so implementing the `#0078d4` accent would REVERSE a deliberate decision — a product-direction reversal (§6 gated). **Surfaced to operator (2026-06-04):** ratify the rebrand (Windows10 stays the Carbon-skinned "MackesDE 10" era; close E4.1, add a checklist pin LOCKING the shared-palette decision; E4.2–E4.20 proceed Carbon-skinned, no Windows-blue) **vs** reverse it (re-introduce a distinct `win10(rgb)` + the retired blue accent across the whole E4 era). Awaiting the direction before touching `palette.rs`.
