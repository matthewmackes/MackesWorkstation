...
- **[>] IN PROGRESS — operator chose "plan + build" (2026-06-04).**
  ...
  **Build sub-tasks (A):** ① add `mde-workbench --panel network:<name>` single-panel launch (open one panel standalone); ② register the 15 in Settings `CATEGORIES` under "Network & Internet" via `Kind::Cmd("mde-workbench --panel network:<name>", false)` (or a new `Kind::Workbench`), reachable as `mde settings --page network:*`; ③ remove the Workbench `Group::Network` (model.rs `enum Group`/`as_str`/`title`, app.rs Message arms + panel fields, patternfly.rs `15 panels`→gone, keyboard.rs:177 `(7, Group::Network)`, home.rs `jump: Some((Group::Network, …))`); ④ Action-Center net toggles → `nm::set_*`; ⑤ verify the Workbench exposes no Network group. Cross-cuts **E6** (Workbench reskin). Multi-fire.
