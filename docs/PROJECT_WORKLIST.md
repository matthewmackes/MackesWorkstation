...
- [>] **GF-1.3: Glusterd binds to the Nebula overlay address.** Split 2026-05-24 per §0.12 into two bench-observable sub-tasks (GF-1.3.a + GF-1.3.b) because the original single-task formulation bundled a prerequisite (the overlay-ip publisher in mackesd) with the consumer (the glusterd config rewriter) and neither could ship without the other in a single commit without violating the no-stubs rule.
...
- [ ] **GF-1.3.b: `mackes-glusterd-nebula-bind.service` rewrites `/etc/glusterfs/glusterd.vol` with the overlay-ip bind line + reloads glusterd.** Acceptance: after both mackesd and the bind service have ticked once, `ss -tnlp | grep 24007` shows glusterd bound to the overlay IP, not `0.0.0.0`. **Implementation note (added 2026-05-24):** the worklist originally specified a `glusterd.vol.d/10-nebula-bind.vol` drop-in path; in Fedora 44, glusterd does not honor a drop-in include — only the main `glusterd.vol` file is parsed. The shipping implementation should rewrite the main file's `volume management` block with an idempotent `option transport.socket.bind-address <ip>` line (preserved across upgrades; written only when the overlay-ip file exists so first-boot installs don't break before enrollment). Until this lands, glusterd binds on 0.0.0.0 (per GF-1.2's `%post` enable); the GF-2.x worker shouldn't bootstrap the volume in production-cuts until GF-1.3.b is green.
