...
- [>] **RETIRE-PY.6: RETIRE-PY — Pure-Rust asset installer — drop the `python3` "Asset installer runtime" dep (catalogue.rs)** *(6a DONE 2026-06-04; 6b open — monorepo-ify the Chicago95/orchestrator path)*
  ...
  **Acceptance** (runtime-observable):
    - [ ] `mde install --assets` fetches + stages the asset set with **no** `python3` invoked. *(6a ✓: the Win2k step is native Rust, zero python — verified end-to-end. Box stays open until 6b vendors the Chicago95 orchestrator so the default `--assets` run completes that half.)*
    - [✓] `catalogue.rs` no longer lists `python3` as an asset-installer runtime dependency. *(6a — removed; `tui_setup` comment updated.)*
    - [ ] A fresh install resolves all bundled assets to disk (verifiable file presence). *(6a ✓ for Win2k: theme tree + index.theme + cache written. Box stays open until 6b verifies Chicago95 end-to-end.)*
