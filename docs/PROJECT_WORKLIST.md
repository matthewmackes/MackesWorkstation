...
- [ ] **E0.15: E0 — Migrate the remaining `swaymsg` IPC calls in the workbench panels off sway onto labwc** *(rescue-pass finding, 2026-06-04)*
  ...
  **Acceptance** (runtime-observable):
    - [ ] No workbench panel shells out to `swaymsg`; `rg "swaymsg" crates/workbench` returns nothing (the mackesd tag-cluster workers are out of scope, tracked separately).
    - [ ] The Repair "Reload compositor" action triggers a labwc config re-read (SIGHUP path) and reports success against a running labwc, error-not-panic when absent.
    - [ ] Mouse/Keyboard/Displays libinput changes are written where labwc re-reads them and a reconfigure makes the change observable at runtime.
...
- [✓] **E1.2: E1 — Role-gated mackesd worker subsets + role-gated surface install (desktop surfaces ENOENT on non-Workstation)**
  ...
  **Live (2026-06-04, post-unblock):** with `/var/lib/mde/role.toml` pinned (operator cleared the dir block), the static worker-tier census is now runtime-verified: `mackesd role-workers {lighthouse,server,workstation}` prints **11 / 14 / 30** workers (header states each count), confirming the rank-superset split. *(Workstation was 31 until RETIRE-PY.3 retired the redundant python `clipboard` worker — `clipd_supervisor`+mde-clipd is the sole clipboard daemon.)* *Still pending a running daemon:* the live worker-status-over-Bus listing (acceptance #1's Bus half) and the ENOENT-on-lower-role surface gate (#2) — the box is pinned Workstation and can't be downgraded to exercise the refusal path live; both are covered by the `worker_role` (7) + `role_gate` (4) unit tests.
  **Acceptance** (runtime-observable):
    - [ ] On Lighthouse, `mackesd` supervises only enrollment(CA)+leader+health workers; on Server it additionally runs fleet+meshfs+metrics; on Workstation it adds the voice coordinator + media stack — verifiable via `mackesd`'s worker-status listing over `mde-bus`. *(Static census live-verified 11/14/30 via `role-workers` — 30 post-RETIRE-PY.3; the live-over-Bus listing needs a running mackesd — bench.)*
