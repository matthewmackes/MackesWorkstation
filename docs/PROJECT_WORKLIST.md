...
- [ ] **E0.3.2: E0.3 — Migrate the 5 file-op D-Bus services -> Bus; rewire mde-files**
  ...
  **Registration audit (2026-06-04):** of the 5 named services, **only `Fleet.Files` is actually registered/live** (`register_fleet_files` serves ONLY `FLEET_FILES_OBJECT_PATH`). **Inbox / Outbox / Downloads / FileOperations are dead `#[interface]` scaffolding** (never registered; mde-files/dbus_backend.rs calls them but they never answer). So the *live* migration here is **Fleet.Files only** (unblocked, follows the Nebula pattern); the 4 dead siblings share E0.3.3/.4's **disposition decision** (delete-as-cruft vs keep-as-Phase-G-scaffold — see the ⚑ audit on E0.3). Don't "migrate" the 4 dead ones blindly.
...
- [ ] **E0.3.4: E0.3 — Migrate the Settings D-Bus service -> Bus** *(UNBLOCKED 2026-06-04: migrate — real get/set impl → Bus responder + register; makes the never-registered service live. The BIGGER one: 5 methods + arg-passing via request body (get/set key, restore json) + the mde-workbench backend.rs `Backend`/`DBusBackend` consumer + its write-through wrapper. The `changed` signal is droppable — never emitted (service never registered). Do in a focused fire.)*
  **As** a surface, **I want** `dev.mackes.MDE.Settings` over Bus. *Reuse:* mackesd/src/ipc/settings.rs. *Deps:* E0.3.1.
  **Audit (2026-06-04):** `settings.rs::SettingsService` has a real `get`/`set` impl but is **never registered** (no `register_*` call) → the interface is never served. The consumer (mde-workbench `backend.rs` `#[proxy] Settings` get/set) calls a never-answered interface. So, like Fleet, there's **no live D-Bus to migrate** — same delete-vs-keep-as-scaffold disposition decision. Unlike Fleet it's implemented, so "keep + register-on-the-Bus" is the more likely intent — but that's the operator's call. *Blocked* pending that decision.
