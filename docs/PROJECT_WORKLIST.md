...
- [ ] **E4.20: E4 — Retire mde-portal: confirm every portal function is reachable as a Win10 idiom (shell/Settings/Explorer/Action Center), then delete the crate**
  ...
  **Remaining (operator chose "finish E4.20" — 2026-06-04):**
  - **① migrate the `mde://` open-uri consumer to a Win10 surface** (scoped — coherent multi-file increment, must land together). The consumer is mde-portal `bus_responder.rs::dispatch_action`: it parses `mde://` URIs (`uri.rs` `parse_mde_uri` → `enum Action`, self-contained: only `std::path`) and dispatches — `OpenApp(id)`→`gtk-launch id`, `OpenFile(path)`→`xdg-open path`, `Lock`→lock, `ToggleDnd`, `Restart`, `Goto(layer)`. **Plan:** (a) move `uri.rs` (`Action`/`parse_mde_uri`/`action_to_uri` + tests) into the shell (`crates/shell/mde/src/uri.rs`); (b) add an `open_uri` consumer — a background `Persist`-poll thread on `action/shell/open-uri` that dispatches the parsed `Action` to **Win10** side effects (OpenApp→gtk-launch, OpenFile→xdg-open, Lock→`mde lock`, ToggleDnd→notifyd, Restart→`systemctl --user restart mde-session`, Goto→task-view); (c) host the thread in **`mde-session`** (the long-running session process — execs+waits on labwc, so it persists for the whole session, matching the portal's persistence). `mde-open.desktop` stays (it's the publisher); only the consumer moves. The other 5 shell verbs (goto/focus/lock/toggle-dnd/restart) are already covered by Win10 idioms (`mde lock`, action-center DnD, task-view) or dead (no Win10 publisher) — only `open-uri` needs the live consumer.
