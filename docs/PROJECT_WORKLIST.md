...
- [ ] **E4.20: E4 — Retire mde-portal: confirm every portal function is reachable as a Win10 idiom (shell/Settings/Explorer/Action Center), then delete the crate**
  ...
  **Remaining (operator chose "finish E4.20" — 2026-06-04):**
  - **① DONE (2026-06-04) — simplified to direct-dispatch, no Bus consumer needed.** Re-checked: **mackesd does NOT publish to `action/shell/*`** (grep clean) — the portal's Bus indirection existed only because the *portal* (a single stateful GUI process) had to do the dispatch. Win10's dispatch is **stateless process spawns**, so the cleanest migration is on-demand: **moved `uri.rs`** (`Action` + `parse_mde_uri`, self-contained — dropped the portal-only `action_to_uri`) into `crates/shell/mde/src/uri.rs`, and added **`mde open-uri <mde://…>`** (`open_uri.rs`) — the `x-scheme-handler/mde` handler that parses + dispatches directly: `OpenApp`→`gtk-launch`, `OpenFile`→`xdg-open`, `Lock`→`mde lock`, `Restart`→`systemctl --user restart mde-session`, `ToggleDnd`→`mde action-center`, `Focus`→no-op (wlr owns focus), `Goto{library→mde files, control/network→mde settings, voip→mde phone, hub→mde start-win10}`. **`mde-open.desktop` repointed** `Exec=/usr/bin/mde open-uri %u` (was `mde-open`). Runtime-verified: `mde open-uri mde://app/X` spawns `gtk-launch X`; unknown/usage exit 1/2 cleanly; 12 moved uri tests pass; clippy/fmt green. This drops the portal's `mde-open` bin + the `bus_responder.rs` entirely (no Bus round-trip) — both go with the crate deletion.
