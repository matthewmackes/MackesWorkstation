...
- [ ] **E4.20: E4 — Retire mde-portal: confirm every portal function is reachable as a Win10 idiom (shell/Settings/Explorer/Action Center), then delete the crate**
  ...
  **Remaining (operator chose "finish E4.20" — 2026-06-04):**
  ...
  - **② retire `mde-portal.service`** (superseded by `mde-session`) + reconcile `mde-open.desktop` (consumer now in mde-session).
  - **③ [operator-gated §0.5]** delete `crates/legacy/mde-portal/` + drop its 6 `lint-design-tokens.allowlist` `portal_full_main.rs` entries.
  - **④** `cargo check --workspace` green (already true — not a member).
  **The crate deletion (③) needs operator authorization; ① is a genuine prerequisite.**
