...
- [ ] **E4.22: E4 — Wire the `mde-popover which-key` binding-mode overlay (never-spawned dead surface)**
  **Discovered during E4.20 crate deletion (2026-06-04).** `mde-popover which-key` (ANIM-6.c / Q55) is a monorepo-era surface that **was never wired to a spawner** (confirmed absent from all three heritage repos) — `MDE_SWAY_MODE` has no setter anywhere, and no `exec`/spawn launches it on a sway `ModeChanged`. The surface itself is complete (parses `~/.config/sway/config` mode blocks, renders the binding card). Two fixes needed: (a) spawn it on sway binding-mode entry with `MDE_SWAY_MODE` set + kill on exit; (b) change `KeyboardInteractivity::Exclusive` → non-grabbing, since an exclusive grab swallows the very `bindsym`s (h/j/k/l in resize mode) the overlay exists to display.
  **As** an operator, **I want** the binding-mode keys shown when I enter sway resize mode, **so that** I can see the available bindings while using them.
  *Deps:* none (independent of the portal — pre-existing gap). *Reuse:* the surface is built; this is wiring + one layer-shell flag.
  **Acceptance** (runtime-observable):
    - [ ] Entering sway resize mode (`$mod+r`) spawns `mde-popover which-key` with `MDE_SWAY_MODE=resize`; leaving the mode dismisses it.
    - [ ] The resize keys (h/j/k/l/Return/Escape) still reach sway while the overlay is up (no exclusive keyboard grab).
    - [ ] `cargo check/build --workspace` green; clippy/fmt clean.
