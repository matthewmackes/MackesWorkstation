...
- [>] **E1.3: E1 — Role-aware systemd units + /etc/mackesd/ templates** *(session=ship-2026-06-04)*
  ...
  **Remaining (RPM-packaging + bench):** the headless unit set (greetd enablement under mde-desktop, lizardfs + ansible-pull.timer under mde-headless `--min-rank 1`) doesn't exist yet — it's authored with the E8 RPM spec. Acceptance **#3** (mackesd reads `/etc/mackesd/` templates → edit+restart changes runtime behavior) is **bigger than a path-repoint**: scoped 2026-06-04, mackesd has *no* `/etc/mackesd/` config loader today (no `mackes-config` dep, no `/etc/mackesd` reads — only per-feature `*Config` structs), so #3 = **build** the loader (a config struct + a startup read of `/etc/mackesd/mackesd.toml` + wire one setting to a worker/relay knob that's observable over the Bus), then verify the edit+restart live (bench). Acceptance #1/#2/#4's *live* checks (`systemctl is-active` per role; a forbidden unit refusing at start) need a role-pinned, unit-installed box — a bench/deployed verification.
