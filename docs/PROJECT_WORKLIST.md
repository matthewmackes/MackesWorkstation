...
- [>] **E4.20: E4 — Retire mde-portal: confirm every portal function is reachable as a Win10 idiom (shell/Settings/Explorer/Action Center), then delete the crate**
  **Crate DELETED + cleaned (2026-06-04, operator-authorized). Holds `[>]` for the two orphans found mid-deletion → E4.21/E4.22.**
  ...
  **Acceptance** (runtime-observable):
    - [>] A mapping confirms each mde-portal function (status/card-index/workspace/URI-open/Bus-responder/**Bus-announce→urgent-theater**) is reachable from a live surface. **MET except the urgent-theater spawn → E4.21.**
...
- [ ] **E4.21: E4 — Re-home the BUS-2.5 urgent theater spawner (mde-portal Dock → live shell process)**
  **Discovered during E4.20 crate deletion (2026-06-04).** The deleted `mde-portal/src/app.rs` held the **only live `fleet/announce` Bus subscription** (`workspace::bus_announce_subscription` → `Message::BusAnnounceSegment` → `spawn_urgent_theater`) that launched `mde-popover urgent` (full-screen theater + title/body/actions via `MDE_URGENT_*` env) on `priority=urgent` segments. mde-bus's server-side dispatch is deliberately `LogOnlySurfaces` (placeholder); the real spawn was always client-side. With the portal gone, **nothing subscribes `fleet/announce` to spawn the theater** — the surface (`mde-popover urgent`) and its mde-bus `Surfaces` trait still exist, only the spawner is orphaned. `mde-panel` (the Dock's successor) does **not** link `mde-bus` today.
  ...
  *Reuse (§2.7):* port the deleted `spawn_urgent_theater` + the `fleet/announce` GFS-poll subscription verbatim into a persistent shell process — **mde-panel** is the natural host (Dock successor, always running). *Deps:* E4.20.
  **Acceptance** (runtime-observable):
    - [ ] A live persistent process subscribes to the `fleet/announce` Bus topic and, on a `priority=urgent` segment, spawns `mde-popover urgent` with `MDE_URGENT_TITLE`/`MDE_URGENT_BODY`/`MDE_URGENT_ACTIONS` set (best-effort, dedup per ULID).
    - [ ] Publishing an `urgent` segment (`mde-bus publish … --priority urgent`) raises the theater on screen; non-urgent priorities do **not**.
    - [ ] `cargo check/build --workspace` green; the spawner's host links `mde-bus`; clippy/fmt clean.
