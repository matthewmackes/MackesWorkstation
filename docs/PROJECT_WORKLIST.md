...
- [>] **E0.3.1: E0.3 — Migrate the Nebula.Status D-Bus service -> Bus (SelfNode / ListPeers)** *(strangler increment landed; full retirement = E0.3.1.a)*
  ...
  **Status (2026-06-03):** Bus **read path established + proven** — `mackesd` boots a named `nebula-bus-responder` thread (own current-thread runtime, `Persist`/rusqlite isn't `Send`) serving `action/nebula/{status,self-node,list-peers}` via `build_reply` over the shared `build_status_snapshot`/`build_peer_list`/`build_self_node` builders; 101 mackesd tests green incl. the responder round-trip. **mde-wizard/preview migrated** to the Bus (`request_verb` → `mde_bus::rpc::request`, 2 s timeout). The 3 D-Bus read verbs (`Status`/`ListPeers`/`SelfNode`) are **kept dual-served** on the `#[interface]` because live consumers (mde-files/mesh_backend, mde-workbench/{mesh_control,home}) still read them — same strategy already applied to `RegenCerts`/`Enroll`/the signals. **No regression:** a prior workflow had deleted the read verbs while leaving those consumers, which would break them against a live daemon (method-not-found); the verbs are restored. Full retirement (migrate the 3 consumers + drop the verbs + drop the allowlist entry) is carved out as **E0.3.1.a**.

- [ ] **E0.3.1.a: E0.3 — Migrate the 3 remaining Nebula.Status READ consumers -> Bus + retire the D-Bus reads**
  **As** a platform dev, **I want** the last D-Bus readers of `dev.mackes.MDE.Nebula.Status` moved onto the Bus, **so that** the `Status`/`ListPeers`/`SelfNode` verbs can be removed and the read path is Bus-only. *Reuse:* mde-wizard/preview's `request_verb` pattern (open `Persist` → block_on `mde_bus::rpc::request("action/nebula/<verb>")` → reuse the existing JSON parsers — reply shapes are identical). *Deps:* E0.3.1. *Consumers to migrate:*
    - `crates/services/mde-files/src/mesh_backend.rs` — `nebula_status`/`nebula_peers`/`nebula_self_node` (currently `zbus::Proxy` `call_string` to Status/ListPeers/SelfNode); needs `mde-bus` dep; keep `parse_nebula_*`.
    - `crates/workbench/mde-workbench/src/panels/mesh_control.rs:625` — `read_nebula_self_node()` (currently shells `dbus-send … SelfNode`); needs `mde-bus` dep; keep the SelfNode parser.
    - `crates/workbench/mde-workbench/src/panels/home.rs:520` — `probe_nebula()` (currently `dbus_call(... "Status")`; it is `async` so call `rpc::request` directly); keep `extract_json_string_field("active_transport")`.
  **Acceptance** (runtime-observable): all 3 consumers read over `action/nebula/{status,self-node,list-peers}` (no `dev.mackes.MDE.Nebula.Status` D-Bus call/`dbus-send` anywhere); the `status`/`list_peers`/`self_node` `#[interface]` methods are removed from nebula.rs; `lint-dbus-shape` allowlist drops nebula.rs' read entry (writes `RegenCerts`/`Enroll` + signals stay until their own substep). *(Round-trip = Bus bench.)*
