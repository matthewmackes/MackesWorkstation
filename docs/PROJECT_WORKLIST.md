...
- [ ] **E4.15: E4 — Networking: panel net-flyout (Wi-Fi list/connect, Airplane) + Settings pages (Status/Wi-Fi/Ethernet/VPN/Mobile-hotspot/Proxy/Data-usage/Airplane) + Action-Center toggles; MIGRATE the 13 Workbench Network panels into Settings > Network here**
  **PARTIAL — the genuine remaining E4 work, NOT an audit-close (scoped 2026-06-04).** Done: the **9 native Network pages** are in the `CATEGORIES` registry (`--list`: Connections/Wi-Fi/Ethernet/VPN/Airplane/Mobile-hotspot/Proxy/Cellular/Data-usage), and the taskbar net-flyout (`net_flyout.rs`, bullet #1) exists. **NOT done — real cross-crate migration:** the **13 Workbench Network panels are still in `mde-workbench`** (`app.rs` still wires `MeshFederationPanel`/`MeshTopologyPanel`/`network_hosts_panel`/… as live Workbench surfaces), and the Settings registry has **no `network:*` migrated entries** — so bullet #2 (the 13 panels registered under Settings ▸ Network) and bullet #3 ("the Workbench no longer exposes any Network group") are unmet. This is genuine implementation that touches BOTH `mde-workbench` (extract/remove the Network group) AND `settings.rs` (register the 13 as `Kind`/pages) + cross-references **E6** (Workbench reskin). Needs a dedicated effort (likely a `/plan` pass on the panel inventory), not a `/ship` audit-close — alongside E4.2 (Bus-client) + E4.20 (mde-portal retire) as the remaining real E4 work.
