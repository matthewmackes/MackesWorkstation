...
#  Per locked decision #7 nothing is committed to this repo; we clone at
#  install time so the upstream license travels with the bytes. This is the
#  one remaining asset-installer script (RETIRE-PY.6b) — the Win2k icon step
#  is now native Rust (`install_win2k.rs`, RETIRE-PY.6a), so no `python3` is
#  involved anywhere in the asset path. `mde install --assets` runs Chicago95
#  here and then the native Win2k step.
# ============================================================================
set -euo pipefail

REPO_URL="https://github.com/grassmunk/Chicago95.git"
CACHE="${XDG_CACHE_HOME:-$HOME/.cache}/mde"
SRC="$CACHE/Chicago95"
...
# Sound theme (the classic login chime + system event sounds)
