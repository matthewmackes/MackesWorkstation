#!/usr/bin/env bash
# Capture the Mackes Workstation PREVIEW GALLERY: a screenshot of every shell component,
...
shot about            about
shot birthright       --wait 6.5 birthright
...
if command -v montage >/dev/null 2>&1; then
    ...
    if (( ${#win10_p0[@]} )); then
        montage "${win10_p0[@]}" \
            ...
            -title "Mackes Workstation — Windows 10 era" \
...
if command -v montage >/dev/null 2>&1; then
    ...
    montage "$out"/panel.png "$out"/start-menu.png "$out"/files.png \
            ...
            -tile 2x -geometry 640x480+6+6 -background '#d4d0c8' -title "Mackes Workstation — preview gallery" \
