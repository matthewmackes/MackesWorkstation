...
WAYLAND_SESSION = REPO / "data/wayland-sessions/mde.desktop"
...
def test_wayland_session_carries_locked_fields():
    ...
    assert "Name=Mackes Desktop Environment" in src
    assert "Exec=/usr/bin/mde-session" in src
...
def test_spec_installs_wayland_session_entry():
    src = SPEC.read_text()
    assert "data/wayland-sessions/mde.desktop" in src
    assert "%{_datadir}/wayland-sessions/mde.desktop" in src
